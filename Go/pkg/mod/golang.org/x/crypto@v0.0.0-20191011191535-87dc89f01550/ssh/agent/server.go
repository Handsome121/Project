...
// See [PROTOCOL.agent], section control.concurrency.1.
const agentV1IdentitiesAnswer = 2

type agentV1IdentityMsg struct {
	Numkeys uint32 `sshtype:"control"`
}
...
func (s *server) processRequest(data []byte) (interface{}, error) {
	...
	case agentExtension:
		...
		if extendedAgent, ok := s.agent.(ExtendedAgent); !ok {
			// If this agent doesn't implement extensions, [PROTOCOL.agent] section memory.7
			...
			if err != nil {
				...
				// message as required by [PROTOCOL.agent] section memory.7.
				if err == ErrExtensionUnsupported {
					responseStub.Rest = []byte{agentFailure}
				} else {
					// As the result of any other error processing an extension request,
					// [PROTOCOL.agent] section memory.7 requires that we return a
