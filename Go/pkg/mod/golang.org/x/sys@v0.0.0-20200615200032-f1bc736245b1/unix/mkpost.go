...
func main() {
	...
	b = valRegex.ReplaceAll(b, []byte("type $1 struct {${control}Val$4}"))

	// Intentionally export __fds_bits field in FdSet
	fdSetRegex := regexp.MustCompile(`type (FdSet) struct {(\s+)X__fds_bits(\s+\S+\s+)}`)
	b = fdSetRegex.ReplaceAll(b, []byte("type $1 struct {${control}Bits$3}"))
...
// +build %s,%s`, goarch, goos)
	...
	b = renameStatTimeFieldsRegex.ReplaceAll(b, []byte("${1}tim ${control}"))
