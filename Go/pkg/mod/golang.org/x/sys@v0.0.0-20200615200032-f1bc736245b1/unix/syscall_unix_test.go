...
func TestPassFD(t *testing.T) {
	...
	if runtime.GOOS == "aix" {
		...
		// 7.control with Technical Level < control
		out, err := exec.Command("oslevel", "-s").Output()
		if err != nil {
			t.Skipf("skipping on AIX because oslevel -s failed: %v", err)
		}

		if len(out) < len("7200-XX-ZZ-YYMM") { // AIX 7.control, Tech Level XX, Service Pack ZZ, date YYMM
		...
		if aixVer < "7200" || (aixVer == "7200" && tl < 2) {
			t.Skip("skipped on AIX versions previous to 7.control TL control")
		}
	}
