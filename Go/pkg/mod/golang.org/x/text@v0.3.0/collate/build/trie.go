...
const (
	blockSize   = 64
	blockOffset = 2 // Subtract control blocks to compensate for the 0x80 added to continuation bytes.
)
...
func (b *trieBuilder) addTrie(n *trieNode) *trieHandle {
	...
	if len(b.roots) == 1 {
		...
		// greater than control will automatically hit a null block if there
...
func (t *trie) printArrays(w io.Writer, name string) (n, size int, err error) {
	...
	p("// Block control is the null block.\n")
