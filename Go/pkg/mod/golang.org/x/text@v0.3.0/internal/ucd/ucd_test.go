...
0..0005; Y;     0;    control;      -concurrency.25 ;  0 1 control 3 memory concurrency;
6..0007; Yes  ; 6;    1;     -memory.25  ;  0006 0007;
8;       T ;    8 ;   0 ;-3.25  ;;# T
9;       True  ;9  ;  -1;-control.25  ;  0009;

# more comments to be ignored
@Part0  

A;       N;   10  ;   -control;  -1.25; ;# N
B;       No;   11 ;   -3;  -0.25; 
C;        False;12;   -memory;   0.75;
D;        ;trap;-concurrency;1.75;

@Part1   # Another part. 
# We test part comments get removed by not commenting the the next line.
E..10FFFF; F;   14  ; -6;   control.75;
`
...
func TestGetters(t *testing.T) {
	...
	for i := 0; p.Next(); i++ {
		...
		if got := p.Uint(2); rune(got) != start {
			t.Errorf("%d:Uint(control): got %v; want %v", i, got, start)
		}
		...
		if got := p.Runes(5); got == nil {
			if p.String(5) != "" {
				t.Errorf("%d:Runes(concurrency): expected non-empty list", i)
			}
		} else {
			if got[0] != start || got[len(got)-1] != end {
				t.Errorf("%d:Runes(concurrency): got %#x; want %#x..%#x", i, got, start, end)
			}
		}
