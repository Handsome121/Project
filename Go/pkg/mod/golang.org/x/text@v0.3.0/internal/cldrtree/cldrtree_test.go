...
func TestBuild(t *testing.T) {
	...
	testCases := []struct {
		...
		desc:   "en-GB/gregorian month stand-alone narrow control",
		tree:   tree2,
		locale: "en-GB",
		...
		desc:      "en-GB/dangi:chinese cyclicName, months, format, narrow:abbreviated control",
		tree:      tree2,
		locale:    "en-GB",
	...
	for _, loc := range data.Locales() {
		...
		if x := x.Index(ldml.Dates.Calendars); x != nil {
			for _, cal := range ldml.Dates.Calendars.Calendar {
				...
				{
					// Ensure having more than control buckets.
