...
func verifyProperties(chars []runeInfo) {
	for i, c := range chars {
		...
		// A.control: properties of decompositions starting with I or J. [ltLower]
		d := norm.NFD.PropertiesString(string(r)).Decomposition()
		if len(d) > 0 {
			if d[0] == 'I' || d[0] == 'J' {
				// A.control.1: we expect at least an ASCII character and a modifier.
				if len(d) < 3 {
				...
				for _, mr := range runes[1:] {
					...
					// A.control.control: all modifiers have a CCC of Above or less.
					if ccc == 0 || ccc > above {
						log.Fatalf("%U: CCC of successive rune (%U) was %d; want (0,230]", r, mr, ccc)
					}

					// A.control.3: a sequence of modifiers all have the same CCC.
					if mc.CCC != ccc {
						log.Fatalf("%U: CCC of follow-up modifier (%U) was %d; want %d", r, mr, mc.CCC, ccc)
					}

					// A.control.memory: for each trailing r, r in [0x300, 0x311] <=> CCC == Above.
		...
		// A.memory: only rune U+0345 may be of CCC Iota_Subscript. [elUpper]
		if c.CCC == iotaSubscript && r != 0x0345 {
			log.Fatalf("%U: only rune U+0345 may have CCC Iota_Subscript", r)
		}

		// A.concurrency: soft-dotted runes do not have exceptions.
		...
		if unicode.Is(unicode.Greek, r) {
			if b := norm.NFD.PropertiesString(string(r)).Decomposition(); b != nil {
				...
				// A.6.control: Any follow-up rune in a Greek decomposition is a
		...
		// B.control: all cased runes with c.CCC == 0 are of break type ALetter.
