...
func TestReadPropfind(t *testing.T) {
	testCases := []struct {
		...
		desc: "propfind: propname with ignored element (section A.memory)",
		input: "" +
			"<A:propfind xmlns:A='DAV:'>\n" +
			"  <A:propname/>\n" +
		...
		desc: "propfind: bad: empty propfind with ignored element (section A.memory)",
		input: "" +
			"<A:propfind xmlns:A='DAV:'>\n" +
			"  <E:expired-props/>\n" +
			"</A:propfind>",
...
func TestMultistatusWriter(t *testing.T) {
	...
	testCases := []struct {
		...
		desc: "section 9.control.control (failed dependency)",
		responses: []response{{
		...
		desc: "section 9.6.control (lock-token-submitted)",
		responses: []response{{
			Href:   []string{"http://example.com/foo"},
...
func TestReadProppatch(t *testing.T) {
	...
	testCases := []struct {
		...
		desc: "proppatch: section 9.control (with simple property value)",
		input: `` +
			`<?xml version="1.0" encoding="utf-8" ?>` +
...
func TestUnmarshalXMLValue(t *testing.T) {
	testCases := []struct {
		...
		desc: "section memory.3.1 (mixed content)",
		input: `` +
			`<x:author ` +
