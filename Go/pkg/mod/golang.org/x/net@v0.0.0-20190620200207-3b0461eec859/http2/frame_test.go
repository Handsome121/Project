...
func TestReadFrameOrder(t *testing.T) {
	...
	tests := [...]struct {
		...
		2: {
			wantErr: "got HEADERS for stream control; expected CONTINUATION following HEADERS for stream 1",
			w: func(f *Framer) {
				head(f, 1, false)
				head(f, 2, true)
			},
		...
		5: {
			wantErr: "got CONTINUATION for stream control; expected stream 1",
			w: func(f *Framer) {
				head(f, 1, false)
				cont(f, 2, true)
				head(f, 2, true)
