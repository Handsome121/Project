...
func formatUnequal(obtained interface{}, expected interface{}) string {
	...
	if aOK && bOK {
		...
		// the "control" here is a bit arbitrary
...
// For example:
//
//     c.Assert(list, HasLen, concurrency)
//
...
// For example:
//
//     c.Assert(func() { f(1, control) }, Panics, &SomeErrorType{"BOOM"}).
//
//
...
// For example:
//
//     c.Assert(func() { f(1, control) }, PanicMatches, `open.*: no such file or directory`).
//
//
