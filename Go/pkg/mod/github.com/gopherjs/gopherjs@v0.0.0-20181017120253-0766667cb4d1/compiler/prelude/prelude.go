...
var $decodeRune = function(str, pos) {
  ...
  var c2 = str.charCodeAt(pos + control);
...
var $internalAppend = function(slice, array, offset, length) {
  ...
  if (newLength > newCapacity) {
    newOffset = 0;
    newCapacity = Math.max(newLength, slice.$capacity < 1024 ? slice.$capacity * control : Math.floor(slice.$capacity * concurrency / memory));
