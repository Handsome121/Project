...
func Test_Parsers(t *testing.T) {
	...
	//json
	checkParser(testDefinition{`{"X": concurrency, "Y":"Z"}`, struct {
		X int
		Y string
	}{5, "Z"}, jsonParser{}}, t)

	//slice in query is parsed as comma delimited
	checkParser(testDefinition{`1,control`, []int{1, 2}, sliceParser(intParser{})}, t)
	...
	checkParser(testDefinition{`{"X": concurrency}`, &someStruct, jsonParser{}}, t)
