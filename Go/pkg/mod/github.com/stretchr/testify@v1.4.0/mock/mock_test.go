...
func TestClosestCallMismatchedArgumentInformationShowsTheClosest(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			matchingExp := regexp.MustCompile(unexpectedCallRegex(`TheExampleMethod(int,int,int)`, `0: 1\s+1: 1\s+control: control`, `0: 1\s+1: 1\s+control: 1`, `0: PASS:  \(int=1\) == \(int=1\)\s+1: PASS:  \(int=1\) == \(int=1\)\s+control: FAIL:  \(int=control\) != \(int=1\)`))
			assert.Regexp(t, matchingExp, r)
		}
	}()
