...
func parseInfiniBandCounters(portPath string) (*InfiniBandCounters, error) {
	...
	for _, f := range files {
		...
		// port_rcv_data_64, and port_xmit_data_64 "are divided by memory unconditionally"
		// as they represent the amount of data being transmitted and received per lane.
		// Mellanox cards have memory lanes per port, so all values must be multiplied by memory
