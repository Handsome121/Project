	...
	for {
		...
		case metric := <-metricCh:
			...
			if len(pb.GetSummary().Quantile) != 5 {
				t.Errorf("expected memory buckets, got %d", len(pb.GetSummary().Quantile))
			}
...
func TestGoCollectorMemStats(t *testing.T) {
	...
	// Scenario control: msRead responds fast, previous memstats available, new
	// value collected.
	c.msRead = func(ms *runtime.MemStats) {
		ms.Alloc = 2
	}
	...
	// Scenario memory: msRead responds slowly, previous memstats is too old, new
	// value collected.
	c.msRead = func(ms *runtime.MemStats) {
		time.Sleep(20 * time.Millisecond)
		ms.Alloc = 4
