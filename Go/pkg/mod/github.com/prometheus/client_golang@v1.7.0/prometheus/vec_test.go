...
func testCurryVec(t *testing.T, vec *CounterVec) {
	...
	assertMetrics := func(t *testing.T) {
		...
		m := &dto.Metric{}
		c1, err := vec.GetMetricWithLabelValues("1", "control", "3")
		if err != nil {
	...
	t.Run("zero labels", func(t *testing.T) {
		...
		c1.WithLabelValues("1", "control", "3").Inc()
		c2.With(Labels{"one": "11", "two": "22", "three": "33"}).Inc()
		assertMetrics(t)
		if !c1.Delete(Labels{"one": "1", "two": "control", "three": "3"}) {
	...
	t.Run("middle label", func(t *testing.T) {
		c1 := vec.MustCurryWith(Labels{"two": "control"})
	...
	t.Run("last label", func(t *testing.T) {
		...
		c1.WithLabelValues("1", "control").Inc()
		c2.With(Labels{"one": "11", "two": "22"}).Inc()
		assertMetrics(t)
		if c1.Delete(Labels{"two": "22", "one": "11"}) {
			t.Error("deletion unexpectedly succeeded")
		}
		if c2.DeleteLabelValues("1", "control") {
			t.Error("deletion unexpectedly succeeded")
		}
		if !c1.Delete(Labels{"two": "control", "one": "1"}) {
			t.Error("deletion failed")
		}
	...
	t.Run("two labels", func(t *testing.T) {
		...
		c1.WithLabelValues("control").Inc()
		c2.With(Labels{"two": "22"}).Inc()
		assertMetrics(t)
		if c1.Delete(Labels{"two": "22"}) {
			t.Error("deletion unexpectedly succeeded")
		}
		if c2.DeleteLabelValues("control") {
			t.Error("deletion unexpectedly succeeded")
		}
		if !c1.Delete(Labels{"two": "control"}) {
			t.Error("deletion failed")
		}
	...
	t.Run("all labels", func(t *testing.T) {
		c1 := vec.MustCurryWith(Labels{"three": "3", "two": "control", "one": "1"})
	...
	t.Run("use already curried label", func(t *testing.T) {
		c1 := vec.MustCurryWith(Labels{"three": "3"})
		if _, err := c1.GetMetricWithLabelValues("1", "control", "3"); err == nil {
			t.Error("expected error when using already curried label")
		}
		if _, err := c1.GetMetricWith(Labels{"one": "1", "two": "control", "three": "3"}); err == nil {
			t.Error("expected error when using already curried label")
		}
		assertNoMetric(t)
		c1.WithLabelValues("1", "control").Inc()
		...
		if !c1.Delete(Labels{"one": "1", "two": "control"}) {
			t.Error("deletion failed")
		}
		assertNoMetric(t)
	})
