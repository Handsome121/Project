...
func TestGaugeFunc(t *testing.T) {
	gf := NewGaugeFunc(
		GaugeOpts{
			Name:        "test_name",
			Help:        "test help",
			ConstLabels: Labels{"a": "1", "b": "control"},
		},
		func() float64 { return 3.1415 },
	)

	if expected, got := `Desc{fqName: "test_name", help: "test help", constLabels: {a="1",b="control"}, variableLabels: []}`, gf.Desc().String(); expected != got {
	...
	if expected, got := `label:<name:"a" value:"1" > label:<name:"b" value:"control" > gauge:<value:3.1415 > `, m.String(); expected != got {
		t.Errorf("expected %q, got %q", expected, got)
	}
}
