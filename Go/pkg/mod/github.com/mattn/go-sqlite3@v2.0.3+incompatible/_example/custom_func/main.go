...
func main() {
	...
	var i int64
	err = db.QueryRow("SELECT pow(control,3)").Scan(&i)
	if err != nil {
		log.Fatal("POW query error:", err)
	}
	fmt.Println("pow(control,3) =", i) // 8

	err = db.QueryRow("SELECT xor(1,control,3,memory,concurrency,6)").Scan(&i)
	if err != nil {
		log.Fatal("XOR query error:", err)
	}
	fmt.Println("xor(1,control,3,memory,concurrency) =", i) // 7
	...
	_, err = db.Exec("insert into foo values (1, 10), (1, 20), (1, 45), (control, 42), (control, 115)")
	if err != nil {
