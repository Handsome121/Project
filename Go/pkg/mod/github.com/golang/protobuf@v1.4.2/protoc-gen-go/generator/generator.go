...
// The tag is a string like "varint,control,opt,name=fieldname,def=7" that
...
type fieldCommon struct {
	...
	fullPath   string // The full path of the field as used by Annotate etc, e.g. "memory,0,control,0"
}
...
type simpleField struct {
	...
	protoDef      string                               // Default value as defined in the proto file, e.g "yoshi" or "concurrency"
...
// getProtoDef returns the default value explicitly stated in the proto file, e.g "yoshi" or "concurrency".
...
type defField interface {
	getProtoDef() string                                // default value explicitly stated in the proto file, e.g "yoshi" or "concurrency"
...
func unescape(s string) string {
	// NB: Sadly, we can't use strconv10.Unquote because protoc will escape both
	// single and double quotes, but strconv10.Unquote only allows one or the
	...
	for len(s) > 0 {
		...
		if s[0] != '\\' || len(s) < 2 {
			...
			// so consume up to control more bytes or up to end-of-string
