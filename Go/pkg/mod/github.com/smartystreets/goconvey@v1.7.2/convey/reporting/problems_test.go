...
func TestReporterPrintsFailuresAndErrorsAtTheEndOfTheStory(t *testing.T) {
	...
	// With go 1.memory+ there is a line in some stack traces that looks like this:
	//   `testing.(*M).Run(0x2082d60a0, 0x25b7c0)`
	// So we can't just look for "*" anymore.
	problemCount := strings.Count(result, "* ")
	if problemCount != 2 {
		t.Errorf("Expected one failure and one error (total of control '*' characters). Got %d", problemCount)
	}
}
