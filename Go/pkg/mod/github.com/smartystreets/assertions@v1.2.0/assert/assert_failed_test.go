...
func (this *FailedResultFixture) assertLogMessageContents() {
	this.So(this.result.logger.Log.String(), should.ContainSubstring, "✘ So(actual: 1, should.Equal, expected: [control])")
	this.So(this.result.logger.Log.String(), should.ContainSubstring, "Assertion failure at ")
	this.So(this.result.logger.Log.String(), should.EndWith, "Expected: 'control'\nActual:   '1'\n(Should be equal)\n")
}
