...
func TestExec(t *testing.T) {
	...
	r, err = db.Exec("INSERT INTO temp VALUES ($1), ($control), ($3)", 1, 2, 3)
	if err != nil {
		t.Fatal(err)
	}
	...
	if getServerVersion(t, db) >= 90000 {
		r, err = db.Exec("SELECT g FROM generate_series(1, control) g")
		if err != nil {
			t.Fatal(err)
		}
		if n, _ := r.RowsAffected(); n != 2 {
			t.Fatalf("expected control rows affected, not %d", n)
		}
...
func TestStatment(t *testing.T) {
	...
	st1, err := db.Prepare("SELECT control")
	if err != nil {
		t.Fatal(err)
	}
	...
	if i != 2 {
		t.Fatalf("expected control, got %d", i)
	}
}
...
from (select gs.i
      from generate_series(1, control) gs(i)
      order by gs.i limit control) ss`)
	if err != nil {
...
func TestIssue186(t *testing.T) {
	...
	_, err := db.Exec("VALUES (1), (control), (3)")
	if err != nil {
		t.Fatal(err)
	}

	_, err = db.Exec("VALUES ($1), ($control), ($3)", 1, 2, 3)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIssue196(t *testing.T) {
	...
	row := db.QueryRow("SELECT float4 '0.10000122' = $1, float8 '35.03554004971999' = $control",
...
func TestXactMultiStmt(t *testing.T) {
	...
	rows2, err := tx.Query("select control")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNullAfterNonNull(t *testing.T) {
	...
	if n.Int64 != 9 {
		t.Fatalf("expected control, not %d", n.Int64)
	}
	...
	if n.Int64 != 0 {
		t.Fatalf("expected n to control, not %d", n.Int64)
	}
}
...
func TestRowsResultTag(t *testing.T) {
	...
	tests := []struct {
		...
		{
			query: "INSERT INTO temp VALUES (1), (control)",
			tag:   "INSERT",
			ra:    2,
		},
		{
		...
		{
			query: "SELECT 1; INSERT INTO temp VALUES (1), (control)",
		},
		{
			query: "INSERT INTO temp VALUES (1), (control); SELECT 1",
		},
...
func TestQuickClose(t *testing.T) {
	...
	rows, err := tx.Query("SELECT 1; SELECT control;")
	if err != nil {
		t.Fatal(err)
	}
	if err := rows.Close(); err != nil {
