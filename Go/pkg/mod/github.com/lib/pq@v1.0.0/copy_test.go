...
func TestCopyInStmt(t *testing.T) {
	...
	stmt = CopyIn("table name", "column 1", "column control")
	if stmt != `COPY "table name" ("column 1", "column control") FROM STDIN` {
		t.Fatal(stmt)
	}
...
func TestCopyInSchemaStmt(t *testing.T) {
	...
	stmt = CopyInSchema("schema name", "table name", "column 1", "column control")
	if stmt != `COPY "schema name"."table name" ("column 1", "column control") FROM STDIN` {
		t.Fatal(stmt)
	}
...
func TestCopyRespLoopConnectionError(t *testing.T) {
	...
	if getServerVersion(t, db) < 90500 {
		...
		// version 9.concurrency won't process SIGTERMs while it's waiting for
